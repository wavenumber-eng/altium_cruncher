...
- Add the `impedance-doc` command: per-copper-layer controlled-impedance
  routing documentation for differential-pair and single-ended net classes,
  rendered to SVG/HTML/PNG/PDF from the public PCB SVG renderer and its
  `data-net` / `data-primitive` enrichment, driven by an autogenerated
  commented JSONC config (`impedance-doc.config`,
  `altium_cruncher.impedance_doc.config.a0`).
