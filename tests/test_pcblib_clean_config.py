...
def _assert_jsonc_properties_are_commented(text: str) -> None:
    ...
    for index, line in enumerate(lines):
        ...
        while previous >= 0 and not lines[previous].strip():
            previous -= 1
        stripped = lines[previous].lstrip() if previous >= 0 else ""
        if previous < 0 or not (stripped.startswith("//") or stripped.startswith("/*")):
            failures.append(line.strip())
    assert failures == []


def test_find_workspace_pcblib_clean_config_path_from_project_child(
    tmp_path: Path,
) -> None:
...
def test_find_workspace_pcblib_clean_config_path_from_config_dir(
    tmp_path: Path,
) -> None:
    config_dir = tmp_path / "config"
    config_dir.mkdir()
