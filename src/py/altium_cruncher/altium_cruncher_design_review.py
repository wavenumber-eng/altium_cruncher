...
def write_design_review_bundle(
    ...
    *,
    ...
    sch_svgs = _write_schematic_svgs(schdoc_paths, output_dir, design)
    doc_jsons = _write_document_jsons([*schdoc_paths, *pcbdoc_paths], output_dir)
...
def _write_notes_json(input_file: Path, output_dir: Path) -> Path:
    output_path = output_dir / "notes" / f"{input_file.stem}_notes.json"
    _write_json(output_path, build_notes_payload(input_file))
    return output_path
...
All paths in `design_review_manifest.json` are relative to `{output_dir}`.
"""
