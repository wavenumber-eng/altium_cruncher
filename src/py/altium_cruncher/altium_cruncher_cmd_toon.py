...
from .pcb_illustration_config import ILLUSTRATION_THEMES, resolve_illustration_config
...
def _resolve_config(
    args: argparse.Namespace, input_file: Path | None = None
) -> PcbSvgConfig:
    ...
    if config_path and not config_path.exists():
        template = resolve_illustration_config(
            theme=args.theme, assembly=args.assembly, pcbdoc=args.pcbdoc
        )
...
def _resolve_inputs(file: str | None) -> list[Path]:
    inputs = (
        [Path(file).resolve()]
        if file
        else find_prjpcbs_in_cwd() or find_pcbdocs_in_cwd()
    )
    if not inputs:
        raise ValueError("Specify a PrjPcb/PcbDoc or run inside its directory")
    for input_file in inputs:
        if not input_file.is_file() or input_file.suffix.lower() not in {
            ".prjpcb",
            ".pcbdoc",
        }:
            raise ValueError(
                f"Input must be an existing PrjPcb or PcbDoc: {input_file}"
            )
    return inputs
...
def register_parser(subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "file", nargs="?", help="PrjPcb/PcbDoc (auto-detected in CWD when omitted)"
    )
    parser.add_argument(
        "-o",
        "--output",
        type=Path,
        default=Path("output/toon"),
        help="output directory (default: ./output/toon)",
    )
    parser.add_argument(
        "--doc", "--pcbdoc", dest="pcbdoc", help="select a PCB within a PrjPcb"
    )
    parser.add_argument(
        "--side",
        choices=("top", "bottom", "both"),
        default="both",
        help="board side (default: both)",
    )
    parser.add_argument(
        "--assembly",
        action="store_true",
        help="add projected assembly designators (red by default)",
    )
    variants = parser.add_mutually_exclusive_group()
    variants.add_argument(
        "--variant",
        help="named PrjPcb variant; omit DNP bodies/labels and apply parameter overrides",
    )
    variants.add_argument(
        "--all-variants",
        action="store_true",
        help="base plus all named variants, each in its own directory",
    )
    parser.add_argument(
        "--theme",
        choices=ILLUSTRATION_THEMES,
    ...
    parser.add_argument(
        "--config",
        type=Path,
        help="SVG JSON/JSONC settings; default: toon.config beside input, created if missing",
    )
    parser.add_argument(
        "--write-config",
        type=Path,
