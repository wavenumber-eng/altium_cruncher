...
def _resolve_project_output_root(output_arg: Path | None) -> Path | None:
    ...
    if output_root.exists() and output_root.is_file():
        raise ValueError(
            f"For .PrjPcb clean, --output must be a directory: {output_root}"
        )
...
def _resolve_project_backup_root(backup_path_arg: Path | None) -> Path | None:
    ...
    if backup_root.exists() and backup_root.is_file():
        raise ValueError(
            f"For .PrjPcb clean, --backup-path must be a directory: {backup_root}"
        )
...
def _render_jsonc_template(
    ...
    comments_by_key = {key: value for key, value in comments.items() if "." not in key}
    return render_commented_jsonc(
        template,
        comments_by_path=comments,
...
def _suffix_from_config_type(config_type: str | None) -> str | None:
    if config_type is None:
        return None
    return (
        _PCBLIB_CONFIG_SUFFIX if config_type == "pcblib" else _SCHEMATIC_CONFIG_SUFFIX
    )
...
def cmd_clean(args: argparse.Namespace) -> int:
    ...
    if suffix == ".prjpcb":
        ...
        try:
            ...
            backup_root = (
                _resolve_project_backup_root(args.backup_path) if args.backup else None
            )
        except ValueError as exc:
            log.error(str(exc))
            return 1
        ...
        for schdoc_path in schdoc_files:
            ...
            output_file = (
                (output_root / rel_path) if output_root else schdoc_path
            ).resolve()
            ...
            if args.backup:
                ...
                if backup_path == schdoc_path:
                    log.error(
                        "Backup path cannot be the same as the input file: %s",
                        schdoc_path,
                    )
                    return 1
        ...
        if success_count != len(schdoc_files):
            log.warning(
                "Clean complete with errors: cleaned=%d/%d",
                success_count,
                len(schdoc_files),
            )
            log.info(
