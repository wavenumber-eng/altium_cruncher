...
_SCHEMATIC_CLEAN_COMMENTS: dict[str, tuple[str, ...]] = {
    "schema": (
        ...
        "The command creates the template and exits before mutating inputs.",
    ),
    "normalize_pin_fonts": (
...
def _write_config_template(path: Path, *, suffix: str) -> None:
    ...
    comments = (
        _PCBLIB_CLEAN_COMMENTS if suffix == ".pcblib" else _SCHEMATIC_CLEAN_COMMENTS
    )
...
def _render_jsonc_template(
    ...
    for index, key in enumerate(keys):
        ...
        value_lines = json.dumps(template[key], indent=2).splitlines()
        comma = "," if index < len(keys) - 1 else ""
        if len(value_lines) == 1:
            lines.append(f'  "{key}": {value_lines[0]}{comma}')
            continue
